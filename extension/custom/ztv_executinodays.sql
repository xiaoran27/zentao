...
-- 项目需求人天统计
-- 取消的任务不计算工时
CREATE OR REPLACE VIEW ztv_executionstroy_days AS 
  select zt_project.id as exec_id, zt_project.name as exec_name
  ...
  where zt_project.deleted = '0'  and zt_project.path like  ',223,%' 
      and zt_project.type in ( 'sprint' ,'stage')
      and DATEDIFF(NOW(), zt_project.begin) <= (365+183)
  group by exec_id, story_id,work_type
  ...
  select zt_project.id as exec_id, zt_project.name as exec_name
      ...
      , 0 as consumed_outer
  from zt_project
      join zt_task on ( zt_project.id = zt_task.execution and zt_task.story = 0 and zt_task.deleted = '0' and zt_task.parent > -1  and not ( zt_task.status = 'cancel' or zt_task.closedReason = 'cancel' )  ) 
...
-- 迭代人天统计
CREATE OR REPLACE VIEW ztv_executiondays AS
  select exec_id, exec_name
    ,sum(bcwp) as bcwp
