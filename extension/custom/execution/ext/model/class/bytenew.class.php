...
class bytenewExecution extends executionModel
{
    ...
    public function getSearchTasks($condition, $pager, $orderBy)
    {
        ...
        $patterns = array();
        $patterns[] = "/FIND_IN_SET\(\s*('[^']*')\s*,\s*(`assignedTo`)\s*\)/i";
        $patterns[] = "/(`assignedTo`)\s+(not\s+)?in\s*\(\s*('[^']*'(?:\s*,\s*'[^']*')*)\s*\)/i" ;
        // if(preg_match($patterns[0], $condition)) {
        //     preg_match($patterns[0], $condition, $matches);
        ...
        // }else 
        if(preg_match($patterns[1], $condition))  //  assignedTo支持in的多选查询 fixed#20860
        {
            preg_match($patterns[1], $condition, $matches);
            $condition = preg_replace('/`(\w+)`/', 't1.`$1`', $condition);
            $condition = str_replace("t1.$matches[0]", "(t1.$matches[0] or (t1.mode = 'multi' and t2.`account` $matches[2] in ($matches[3]) and t1.status != 'closed' and t2.status != 'done') )", $condition);

            $sql = $sql->leftJoin(TABLE_TASKTEAM)->alias('t2')->on("t2.task = t1.id and t2.account $matches[2] in ($matches[3])");

        } else if(strpos($condition, '`assignedTo`') !== false)
        {
