...
//                ->andWhere('status')->eq($status)->andWhere('stage')->eq($stage)
    ...
    public function getTextForDing($type = 'requirement', $product = -1, $sla = 0, $program = 223, $responseResult = 'todo', $excludeUsers = 'admin')
    {
        ...
        $dingdingDatas = $this->dao->select("zu.account as account, zu.realname  as realname , if( ifnull( zu.dingding, '') = '' , zu.mobile , zu.dingding  ) as dingding, count(distinct zs.id) as total, group_concat(distinct zs.id) as ids ")->from(TABLE_STORY)->alias('zs')
    ...
    public function update($storyID)
    {
        ...
        if (!dao::isError()) {
            ...
            if ($this->post->comment != '' or !empty($changes)) {
                ...
                $this->loadModel('common')->log("'story', $storyID, $action,$comment", __FILE__, __LINE__);
        ...
        if ($col->show) {
            ...
            switch ($id) {
                ...
                case 'rspRejectTime':
                    ...
                    break;
                case 'prdReviewTime':
                    echo helper::isZeroDate($story->prdReviewTime) ? '' : substr($story->prdReviewTime, 0, 10);
                    break;
                case 'releaseTime';
                    echo helper::isZeroDate($story->releaseTime) ? '' : substr($story->releaseTime, 0, 10);
                    break;
                case 'asort':
                    echo $story->asort;
                    break;
