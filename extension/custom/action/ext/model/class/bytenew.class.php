<?php
class bytenewAction extends actionModel
{
    ...
    public function create($objectType, $objectID, $actionType, $comment = '', $extra = '', $actor = '', $autoDelete = true, $changes=array())
    {
        ...
        if (!empty($changes))  $this->logHistory($actionID, $changes);
    ...
    public function printAction($action, $desc = '', $onlyReturn=false)
    {
        ...
        if(is_array($desc))
        {
            ...
            if ($onlyReturn) return $actionDesc;
            echo $actionDesc;
        }
        else
        {
            if ($onlyReturn) return $desc;
            echo $desc;
        }
    }
    ...
    public function printChanges($objectType, $histories, $canChangeTag = true, $onlyReturn = false)
    {
        ...
        foreach($histories as $history)
        {
            ...
            if($history->diff != '')
            {
                ...
                $outstr .= sprintf($this->lang->action->desc->diff2, $history->fieldLabel, $history->noTagDiff, $history->diff);
            }
            else
            {
                $outstr .= sprintf($this->lang->action->desc->diff1, $history->fieldLabel, $history->old, $history->new);
            }
        }
        if ($onlyReturn) return $outstr;
        echo $outstr;
    }
