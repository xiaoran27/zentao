...
class dingtalkModel extends model
{
    ...
    public function updateSessionDing($del=false){
        if($del){$this->session->set('ding_state',null); $this->caches('ding_state',null,1); return null;}
        $random = rand(1000,9999);
        $this->session->set('ding_state', $random);
        $this->caches('ding_state',$random,3*60);
    ...
    public function getDingtalkToken(){
        ...
        if(empty($token)){
            $result = $this->curl('https://oapi.dingtalk.com/gettoken?appid='.$this->config->ding->appid.'&appsecret='.$this->config->ding->appsecret,[],'GET');
     ...
     * */
    public function getDingtalkUserInfoByCode($code){
        if(empty($code)) ['errcode'=>500,'errmsg'=>'缺少用户授权码code'];
        $timestamp=ceil(microtime(true)*1000);
        $result = $this->curl('https://oapi.dingtalk.com/sns/getuserinfo_bycode?accessKey='.$this->config->ding->appid.'&timestamp='.$timestamp.'&signature='.base64_encode(hash_hmac('sha256', $timestamp, $this->config->ding->appsecret, true)),json_encode(['tmp_auth_code'=>$code]));
        $res = json_decode($result,true);
        if($res['errcode']!=0) return ['errcode'=>$res['errcode'],'errmsg'=>$res['errmsg']];
        return ['errcode'=>0,'errmsg'=>'ok','user_info'=>$res['user_info']];
    ...
    public function errjump($url,$msg){
        echo "<html><head><meta charset='utf-8'><script>setTimeout(function(){location.href='".$url."'},10000);</script></head>";
        ...
        die("<div style='margin-bottom:8px;'>登录失败! <strong style='color:#ed980f'>".$msg."</strong> <br/>10 秒后页面自动跳转</div>");
    }
	 ...
	 * 汉字转拼音方法
	 * $str 要转换的字符串
	 * $ret_format 转换方式,all全拼,first首字母
	 * $placeholder 标点符号替换符
	 * $ds 转换后拼音之间的间隔符
	 * $allow_chars 允许的不转换的字符
	 **/
	public function pinyin($str, $ret_format = 'all', $placeholder = '_',$ds = '', $allow_chars = '/[a-zA-Z\d ]/') {
        static $pinyins = null;
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($asc < 0x80) { // 0-127
                ...
                if (isset($pinyins[$chr])) {
                    $rs .= 'first' === $ret_format ? $pinyins[$chr][0] : ($pinyins[$chr] . $ds);
                } else {
                    $rs .= $placeholder;
                }
            }
