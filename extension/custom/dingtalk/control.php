...
class dingtalk extends control
{
    ...
    public function login(){
        ...
        if(isset($_GET['code']) and isset($_GET['state'])){
            ...
            if(empty($code)) $this->dingtalk->errjump('/zentao/user-login.html','用户临时码code不能为空');
            if(empty($state)) $this->dingtalk->errjump('/zentao/user-login.html','状态参数state不能为空');


            $statesess = $this->session->ding_state;
            if(empty($statesess)) $statesess =  $this->dingtalk->caches('ding_state')  ;
            // $common->log(json_encode(array('code'=>$_GET['code'], 'state'=>$_GET['state'], 'statesess'=>$statesess, 'session'=>$this->session, 'cookie'=>$this->cookie, '_SESSION'=>$_SESSION),JSON_UNESCAPED_UNICODE), __FILE__, __LINE__);
            // if($statesess != $state) $this->dingtalk->errjump('/zentao/user-login.html','状态参数state过期');

            $usercode = $this->dingtalk->getDingtalkUserToken($code); /* 获取用户的持久授权码 */
            if($usercode['errcode']!=0) $this->dingtalk->errjump('/zentao/user-login.html',$usercode['errmsg']);
            ...
            if($this->session->user !== false){
                ...
                if($usersns['errcode']!=0) $this->dingtalk->errjump('/zentao/user-login.html',$usersns['errmsg']);

                $userinfo = $this->dingtalk->getDingtalkUserInfo($usersns['sns_token']); /* 获取用户的基本信息 */
                if($userinfo['errcode']!=0) $this->dingtalk->errjump('/zentao/user-login.html',$userinfo['errmsg']);
                ...
                if($this->dingtalk->bindDingtalkUser($this->session->user->account,$data)===false){
                    $this->dingtalk->errjump('/zentao/user-login.html','绑定失败');
                }else{
            ...
            if($user){
                ...
                $this->dingtalk->errjump('/zentao/user-login.html','登录失败,用户不存在');
            }

        }else{
            $this->dingtalk->errjump('/zentao/user-login.html','缺少钉钉参数');
        }
