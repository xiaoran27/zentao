...
?>
<?php
class bytenewSearch extends searchModel
{
    ...
    public function conditionsBySelectValue($value, $operator='=')
    {
        ...
        // $this->loadModel('common')->log(json_encode($value,JSON_UNESCAPED_UNICODE) . '=> values=' . json_encode($values,JSON_UNESCAPED_UNICODE), __FILE__, __LINE__);
        if ( $value == 'all' ) return "= 'all' ";
        // if ( $value == 'all' ) return $operator . " 'all' ";

        foreach ( $values as $k=>$v) {
            if ( 'all' == $v  ) {
                return "= 'all' ";  // 选择的有all值,认为是全部
                // return $operator . " 'all' ";
            }
        }
