...
include '../../../../../module/common/view/chosen.html.php';
...
foreach($fieldParams as $fieldName => $param)
{
    ...
    $multiSel = ($fieldName == 'openedBy' || $fieldName == 'assignedTo' || $param['control'] == 'select' || $param['control'] == 'multi-select' )?" multiple ":"";
...
<table class='table table-condensed table-form' id='<?php echo "{$module}-search";?>'>
  <tbody>
    <tr>
        ...
        <table class='table table-form table-fixed'>
          <tbody>
            ...
            for($i = 1; $i <= $groupItems; $i ++)
            {
                ...
                $multiSel = ($currentField == 'openedBy' || $currentField == 'assignedTo' || $param['control'] == 'select' || $param['control'] == 'multi-select' )?" multiple ":"";
      ...
      <td class='text-center nobr w-70px'> <?php echo html::select('groupAndOr3', $lang->search->andor, $formSession['groupAndOr3'], "class='form-control'")?></td>
      <td class='w-400px'>
        <table class='table table-form'>
          <tbody>
            <?php
            for($i = 1; $i <= $groupItems; $i ++)
            {
                ...
                /* Print and or. */
                echo "<td class='text-right w-80px'>";
                if($i == 1) echo "<span id='searchgroup3'><strong>{$lang->search->group3}</strong></span>" . html::hidden("andOr$fieldNO", 'AND');
                if($i > 1)  echo html::select("andOr$fieldNO", $lang->search->andor, $formSession["andOr$fieldNO"], "class='form-control'");
                echo '</td>';
    ...
    <tr>
      ...
      <td colspan='3' class='text-center form-actions'>
        ...
        echo html::hidden('groups', $config->search->groups);
...
var groups    = <?php echo $config->search->groups;?>;
...
$(function()
{
    ...
    var setField = window.setField = function(obj, fieldNO, moduleparams)
    {
        ...
        {
            // 多选的name属性值追加'[]'
            var multiSel = ( fieldName == 'openedBy' || fieldName == 'assignedTo' ) ||  params[fieldName]['control'] == 'select' || params[fieldName]['control'] == 'multi-select';
